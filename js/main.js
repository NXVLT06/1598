...
(function () {
  ...
  // ── 2. Birthday Music Intro + Gokul's Voice Wish Sequence ──
  const voiceAudioEl     = document.getElementById('gokul-voice-audio');
  const voiceSoundwave   = document.getElementById('voice-soundwave');
  const voiceStatusLabel = document.getElementById('voice-status-label');

  let hasSequenceStarted = false;
  let audioCtx = null;

  // Synthesize soft piano/music-box "Happy Birthday" intro melody
  function playBirthdayMusicIntro(onComplete) {
    try {
      ...
      // Happy Birthday Melody Notes (freq in Hz, duration in sec)
      const melody = [
        { note: 392.00, duration: 0.28 }, // G4
        { note: 392.00, duration: 0.28 }, // G4
        { note: 440.00, duration: 0.45 }, // A4
        { note: 392.00, duration: 0.45 }, // G4
        { note: 523.25, duration: 0.45 }, // C5
        { note: 493.88, duration: 0.70 }, // B4
        
        { note: 392.00, duration: 0.28 }, // G4
        { note: 392.00, duration: 0.28 }, // G4
        { note: 440.00, duration: 0.45 }, // A4
        { note: 392.00, duration: 0.45 }, // G4
        { note: 587.33, duration: 0.45 }, // D5
        { note: 523.25, duration: 0.85 }  // C5
      ];
      ...
      melody.forEach((item) => {
        ...
        gain.gain.exponentialRampToValueAtTime(0.001, startTime + item.duration * 0.95);
      ...
      const totalMusicDuration = (startTime - audioCtx.currentTime) * 1000;

      // Trigger Gokul's voice after music completes
      setTimeout(() => {
        if (onComplete) onComplete();
      }, Math.max(500, totalMusicDuration - 200));

    } catch (e) {
      console.log("Music synth fallback:", e);
      if (onComplete) onComplete();
    }
  }
  ...
  if (voiceAudioEl) {
    ...
    // Gesture fallback for browsers with strict autoplay policies
    const userGestureEvents = ['click', 'touchstart', 'scroll', 'pointerdown', 'keydown'];
    const triggerSequenceOnGesture = () => {
    ...
    userGestureEvents.forEach(evt => {
      window.addEventListener(evt, triggerSequenceOnGesture, { passive: true, once: true });
    });

    // Immediate attempt
    setTimeout(playFullBirthdaySequence, 300);
  }
